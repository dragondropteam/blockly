...
/**
 * Modifications made by DigiPen Institute of Technology 2017
 * @author Luke Powell
 * April 2017 - Added in the concept of strict typing making blocks only accept blocks that are typed
 */
 ...
 *     list of statement functionTypes.  Null/undefined if any type could be connected.
 * @param strict Whether this connection is strict allowing only connections that are typed and typed correctly
 */
Blockly.Block.prototype.setPreviousStatement = function (newBoolean, opt_check, strict) {
  if (newBoolean) {
    ...
    this.previousConnection.setCheck(opt_check);
      this.previousConnection.strict_ = strict || false;
  } else {
    if (this.previousConnection) {
 ...
 * @param strict Whether this connection is strict allowing only connections that are typed and typed correctly
 */
Blockly.Block.prototype.setNextStatement = function (newBoolean, opt_check, strict) {
  if (newBoolean) {
    if (opt_check === undefined) {
      opt_check = null;
    }
    ...
    this.nextConnection.setCheck(opt_check);
      this.nextConnection.strict_ = strict || false;
  } else {
    if (this.nextConnection) {
 ...
 *     of returned functionTypes.  Null or undefined if any type could be returned
 *     (e.g. variable get).
 */
...
/**
 * Additions copyright DigiPen Institute of Technology 2016
 */
Blockly.Block.prototype.retypeVar = function(name, type, cType){
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldVariable &&
        Blockly.Names.equals(name, field.getValue())) {
          field.setType(type);
          field.setCType(cType);

          var typeField = this.getField('TYPE');
          if(typeField){
            typeField.setValue(cType);
          }
          
          var typeLowerField = this.getField('TYPELOWER');
          if(typeLowerField){
            typeLowerField.setValue(cType.toLowerCase());
          }
      }
    }
  }
...
Blockly.Block.prototype.getVariableFieldIfExists = function(name){
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldVariable &&
        Blockly.Names.equals(name, field.getValue())) {
