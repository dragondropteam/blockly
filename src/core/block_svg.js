...
/**
 * File modified by DigiPen Insitute of Technology
 * @author luke.powell@digipen.edu (Luke Powell)
 * - Forward help url to main process to open in a BrowserWindow
 * Copyright 2017
 */
...
Blockly.BlockSvg.prototype.snapToGrid = function () {
    ...
    if (!this.workspace.options.gridOptions || !this.workspace.options.gridOptions['snap']) {
...
Blockly.BlockSvg.prototype.showHelp_ = function () {
    ...
    if (url) {
        // Modification by DigiPen Institute of Technology 2017
        const {ipcRenderer} = require('electron');
        ipcRenderer.send('show_help', url);
        // end modifications
        // The below operation does not work on Desktop
        // window.open(url);
    }
};
...
Blockly.BlockSvg.prototype.connectionUiEffect = function () {
    ...
    var xy = Blockly.getSvgXY_(/** @type {!Element} */ (this.svgGroup_),
        this.workspace);
    // Offset the coordinates based on the two connection functionTypes, fix scale.
...
Blockly.BlockSvg.prototype.setPreviousStatement =
    function (newBoolean, opt_check, strict) {
 ...
 *     list of statement functionTypes.  Null/undefined if any type could be connected.
 */
Blockly.BlockSvg.prototype.setNextStatement = function (newBoolean, opt_check, strict) {
